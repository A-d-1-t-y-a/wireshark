...
To address the immediate threat of active external connections, a Bash script was developed to automate the blocking of identified malicious IP addresses.

\subsection{Script Overview: Block Attacker IPs}
The script \texttt{Script1\_Block\_Attacker\_IPs.sh} performs the following defensive actions:
\begin{enumerate}
    \item \textbf{Root Verification:} Ensures the script is run with necessary administrative privileges.
    \item \textbf{Firewall Configuration:} Uses \texttt{iptables} to create DROP rules for both Incoming (INPUT) and Outgoing (OUTPUT) traffic.
    \item \textbf{Targeted Blocking:} Specifically blocks the four hostile IPs identified in the PCAP analysis:
    \begin{itemize}
        \item 203.0.113.5 (C2 Server)
        \item 198.51.100.12 (Exfiltration Server)
        \item 103.20.10.1 (DDoS Source)
        \item 192.0.2.25 (Reconnaissance Source)
    \end{itemize}
    \item \textbf{Verification:} Runs a check to confirm the rules have been successfully applied and displays the current firewall status.
\end{enumerate}

This script satisfies the requirement to "Block an attacker IP from the PCAP using iptables" and effectively severs the attacker's command and control channel.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{script1_output.png}
    \caption{Output of Script 1 showing successful blocking of attacker IPs.}
    \label{fig:script1}
\end{figure}
