...
To address the threat of covert data exfiltration, a Bash script was developed to detect suspicious DNS traffic patterns associated with the attack.

\subsection{Script Overview: Detect DNS Exfiltration}
The script \texttt{Script2\_Detect\_DNS\_Exfiltration.sh} automates the detection of DNS tunneling, a technique used by the attackers to hide C2 communications. The script performs the following actions:
\begin{enumerate}
    \item \textbf{Target Identification:} Defines known malicious domains found in the PCAP (\texttt{syslog-host.cn} and \texttt{updcdn.ru}).
    \item \textbf{Hosts File Check:} Verifies if these domains are already blacklisted in the \texttt{/etc/hosts} file.
    \item \textbf{Log Analysis:} Searches system logs (\texttt{/var/log/}) for any recent queries to these domains.
    \item \textbf{Alerting:} Provides real-time warnings if any recent contact with these malicious domains is found.
\end{enumerate}

This script satisfies the requirement to "Detect DNS exfiltration patterns" and helps in identifying compromised hosts that are attempting to "phone home" via DNS.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth]{script2_output.png}
    \caption{Output of Script 2 showing DNS exfiltration detection.}
    \label{fig:script2}
\end{figure}
