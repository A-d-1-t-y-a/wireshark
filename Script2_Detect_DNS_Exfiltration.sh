...
for domain in "${MALICIOUS_DOMAINS[@]}"; do
    ...
    if [ "$count" -gt 0 ]; then
        echo "    [ALERT] Found $count queries to $domain in logs!"
        DNS_HITS=$((DNS_HITS + 1))
    else
        echo "    [OK] $domain - No queries found in logs"
    fi
done
echo ""
...
if [ $DNS_HITS -gt 0 ]; then
    echo -e "\e[31m[ALERT] DNS exfiltration indicators found!\e[0m"
    echo ""
    ...
    echo -e "\e[32m[SECURE] No DNS exfiltration detected.\e[0m"
fi
